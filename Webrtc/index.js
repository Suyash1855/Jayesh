...
let checkNumberOfUsers = async () => {
  ...
  if (members.length >= 2) {
    // Send alert message to new user (assuming signalling server allows this)
    console.error("Room is full. New user cannot join.");
    // Implement logic to restrict the user from joining the room (e.g., disconnect the user or redirect them)
    // leaveChannel();
    return;
  }
